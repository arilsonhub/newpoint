...
class Validation {
	...
	/**
     *  Método que verifica se pelo menos um valor da lista informada foi preenchido
     *  @return $this (retorna o próprio objeto)
     */
    public function minimoUmValorPreenchido(Array $listaValores) {
	
	    //Verifica se o elemento possui valor, se não possuir então procuramos nos demais elementos da lista
        if (strlen($this->dados['valor']) == 0) {
        	
			//Procura valor nos outros elementos da lista
			foreach($listaValores as $dado){
			  
			   //Trata a String do valor passado
			   $dado = trim($dado);			  
			   //Verifica se o elemento tem valor
			   if(isset($dado[0])){
			       //Tem valor, então retornamos o próprio objeto
			       return $this;
			   }
			}
			
			//Nenhum elemento da lista possui valor, então geramos o erro na validação
        	$mensagem = ($this->encode == true ? utf8_encode($this->dados['mensagem']) : $this->dados['mensagem']);
	...
	public function validarMimeType($uploadhelper,Array $mimelist,$file){
		
		//Verifica se existe valor para comparar
		if(strlen($this->dados['valor']) > 0){
		
			//Valida o mime type do arquivo do curriculo
			if(!$uploadhelper->validateMimeType($mimelist,$file)){
	...
	public function validarTamanhoArquivo($uploadhelper,$file,$maximumsize){
	
	   //Verifica se existe valor para comparar
		if(strlen($this->dados['valor']) > 0){
		
			//Valida tamanho do arquivo
			if($uploadhelper->validateSize($file,$maximumsize)){
