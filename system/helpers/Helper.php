...
class Helper {
	 ...
	 */
	public function getPermalink($limit=null,$offset=null,$length=null,$beforeChar=null){
	
		//Divide a URL
		$requested_permalink = array_slice(explode('/',$_SERVER['REQUEST_URI'],$limit),$offset,$length);
		
		if(count($requested_permalink) > 0){
			
			//Verifica se deseja-se que seja retornado um pedaço da string antes de um dado caracter
			if(!is_null($beforeChar)){
				//Verifica se o caracter existe na String
				if(strpos($requested_permalink[0],$beforeChar) !== false){					
					$requested_permalink[0] = substr($requested_permalink[0],0,stripos($requested_permalink[0],$beforeChar));
				}				
			}
			
			//Retorna o permalink		
			return $requested_permalink;
	 ...
	 * @param String
	 */
	public function ExecuteRegex($pattern,$replacement,$String){
